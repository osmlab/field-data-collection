...
test('create an observation to a pre-existing OSM.org node, and check they are linked', function (t) {
  t.plan(4)
...
test('simulate an observation created before & after a placeholder node becomes an OSM.org node, and check they are linked', function (t) {
  ...
  function createPlaceholderNodeAndObservation (done) {
    ...
    osm.createNode(node, function (err, node) {
      ...
      osm.createObservation(newDoc.tags['osm-p2p-id'], observation, function (err, obsDoc, _linkDoc) {
        t.error(err)
        linkDoc = _linkDoc
        t.equals(linkDoc.obs, obsDoc.id)
