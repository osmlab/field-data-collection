...
module.exports = function(makeOsm) {
  test("refbug", function(t) {
    ...
    function ready() {
      ...
      osm.query(bbox, function(err, res) {
        ...
        var node2 = res.filter(function(doc) {
          return doc.deleted;
        })[0];
  ...
  test("refbug 2: nodes referred to by old versions of a way are retained", function(
    t
  ) {
    var osm = makeOsm();
    ...
    var batch = [
      ...
      { type: "put", key: "C", value: { type: "node", lat: 3.0, lon: 4.0 } },
      {
        type: "put",
        key: "D",
        value: { type: "node", lat: 64.123, lon: -147.56 }
      },
  ...
  test("refbug 2: ways referred to by old versions of a relation are retained", function(
    t
  ) {
    var osm = makeOsm();
    ...
    var batch = [
      ...
      },
      {
        ...
        value: {
          type: "relation",
          members: [{ type: "node", ref: "A" }, { type: "node", ref: "B" }]
        }
      }
    ];
    var versions = {};
    ...
    function ready() {
      osm.refs.list("A", function(err, refs) {
        ...
        t.deepEqual(
          refs.map(mapKeys),
          [rel],
          "A referenced by original relation"
        );
        done();
      });
      osm.refs.list("B", function(err, refs) {
        ...
        t.deepEqual(
          refs.map(mapKeys),
          [rel],
          "B referenced by original relation"
        );
        done();
      });
      osm.refs.list("C", function(err, refs) {
    ...
    function nextStep() {
      ...
      var rel1 = {
        type: "relation",
        members: [{ type: "node", ref: "A" }, { type: "node", ref: "C" }]
      };
